...
- **Behavior** — behavioral driver attached to a node (e.g. `vmc_receiver`, `breathing`, `manual_calibration`, `blendshape_limiter`, `lipsync_processor`, `mediapipe_tracker`, `api_controller`), stored in the `behaviors` table (renamed from `node_components` in migration 022)
