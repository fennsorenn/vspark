...
Defined entirely in [packages/shared/src/signal.ts](packages/shared/src/signal.ts). Node classes are registered in the backend registry and serialized as `GraphDescriptor` objects. The 104 built-in node kinds live under [packages/backend/src/signal/nodes/](packages/backend/src/signal/nodes/).
