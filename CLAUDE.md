...
- **If the web/cloud harness started you on an auto-generated `claude/<gibberish>` branch, work on it directly — do NOT rename it.** The managed git proxy **refuses branch deletion outright** (any `git push --delete` returns HTTP 403, and no GitHub MCP tool deletes a ref), so a rename can never be cleaned up: it just strands a duplicate `feature/…` branch *alongside* the original `claude/…` one, and now you have two un-deletable branches instead of one. Accept the generated name for the session and push your work to it. (Branches can only be removed from outside this environment — the GitHub web UI or a normal git client.)
