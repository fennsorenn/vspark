...
- **Microphone lipsync** — in-browser MFCC vowel classification with per-component calibration, driving `Fcl_MTH_*` viseme weights and jaw-open from RMS.
...
- **Components** — behavioral drivers attached to nodes: VMC receiver, breathing, lipsync, MediaPipe tracker. Each component is backed by its own signal graph instance.
...
- **32 built-in node kinds** including OSC source, bone mappers, body/arm calibration, IK targets, MediaPipe converters, blendshape mux, and broadcast sinks.
- **Visual graph editor** in the frontend for inspecting and wiring components.
...
- **Cross-platform releases** — bundled Node.js 20 runtime, win-x64 and linux-x64 zips built by CI.
...
Prebuilt zips for Windows and Linux are published on the [GitHub Releases](../../releases) page. Each bundle ships with a Node.js 20 runtime, the backend bundle, the frontend assets, and a start/updater script — no Node or pnpm install required.
...
In-app updates: the editor TopBar checks GitHub Releases on the selected channel (stable / pre-release) and runs the bundled updater to apply downloads.
...
Open the frontend in a browser. The backend opens its UDP OSC socket on the port configured per VMC component.
...
The packaged release (with bundled Node runtime and updater scripts) is produced by the GitHub Actions workflow in `.github/workflows/release.yml`.
