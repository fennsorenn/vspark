...
| Standalone project graphs | WIP | `feature/overlive-integration` — `project_graphs` table + REST CRUD + enable-flag lifecycle; writable `SignalGraphCanvas`; standalone graphs cannot use `component_config` / `component_id` / `scene_entity` (throw at runtime). See [project-graphs.md](modules/project-graphs.md). |
| Overlive integration (Twitch + SE) | WIP | `feature/overlive-integration` — `OverliveManager` running shared `OverliveKit` per loaded project; per-project Twitch app credentials + OAuth accounts (refresh tokens) + SE JWT accounts; Accounts modal in TopBar; new `Account` port type; 13 event signal nodes (`overlive_redemption`, `overlive_subscription`, `overlive_gift_bomb`, `overlive_raid`, `overlive_follow`, `overlive_chat_message`, `overlive_chat_command`, `overlive_chat_delete`, `overlive_ad_start`, `overlive_ad_end`, `overlive_ban`, `overlive_stream_online`, `overlive_stream_offline`). See [overlive.md](modules/overlive.md). |
...
- [project-graphs.md](modules/project-graphs.md) — **WIP** — standalone project-scoped signal graphs (writable canvas, enable-flag lifecycle, no component context)
- [overlive.md](modules/overlive.md) — **WIP** — Twitch + StreamElements integration via the `overlive` SDK; accounts, OAuth, `Account` port type, 13 event nodes
