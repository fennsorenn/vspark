...
| Unified sync layer | WIP (core refactored onto @vspark/mesh; app integration pending) | `sync/` + `packages/shared/src/sync.ts` — legacy envelope for historic state. Core abstraction refactored into `@vspark/mesh` (package interface, HLC, channels, acks, snapshots, hydration taps). Remaining work: migrating REST mutation routes to write through the store, frontend store migration to mesh-react bindings, full HLC persistence/reconcile. See [sync.md](modules/sync.md) and [mesh.md](modules/mesh.md). |
...
| Multiplayer / mesh integration | WIP (app integration layer) | Parallel-run backend wiring: five collections persisted via `onCommitted` taps (schema in `packages/backend/src/mesh/index.ts` BINDINGS). Frontend: per-tab peer with stable sessionStorage participant id, auto-subscription re-arming, mounted from `Editor.tsx`, Vite proxy at `/mesh`. UI still reads from Zustand, not mesh bindings. Remaining: REST mutation routes writing through the store, frontend store migration to mesh-react bindings. See [mesh.md](modules/mesh.md) and [multiplayer.md](modules/multiplayer.md). |
