...
| Multiplayer / mesh integration | WIP (object-share migration + REST/frontend bindings pending) | Backend: five collections + collab-scene grants + ServerMeshTransport (`_mesh2` namespaced legacy WebRTC channels), legacy bridge sync.document ↔ mesh replica mirror with echo guard. Frontend: per-tab peer + auto-subscription re-arming mounted from Editor. New: ServerMeshTransport (packages/backend/src/mesh/serverMeshTransport.ts), collab.ts grants/arming logic, mesh_tombstones table (HLC persistence, 30-day prune). **Now: Step D — migrating object-share document-plane (place grants) onto mesh, with receiver persistence skipping foreign projections.** Known: werift stale-slot blocks single-side reconnect (pre-existing), model-swap assets not yet on mesh. Remaining: REST mutation routes → store, UI → mesh-react bindings. See [mesh.md](modules/mesh.md) and [multiplayer.md](modules/multiplayer.md). |
