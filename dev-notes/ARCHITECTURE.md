...
| Runtime overrides bus | WIP (Phase 1, branch `feature/graph-runtime-overrides-spawn-text`) | `runtime_overrides/manager.ts` (new) — backend bus mirroring `broadcastBus`, keyed by `(targetKind, targetId, paramPath)`. Two modes: ephemeral (in-bus only) and persistent (also writes through to scene-node / compose-layer routes). WS: `runtime_override_set`, `runtime_override_clear`, `runtime_override_snapshot` (on connect). Parallel to track-clip overrides; track-clip override wins on conflict for transform/scalar params. See [runtime-overrides.md](modules/runtime-overrides.md). |
| Spawn manager | WIP (Phase 1) | `spawn/manager.ts` (new) — ephemeral clip-clone spawning. On `spawn_clip`, deep-clones the owner of a clip (always unhidden), duplicates the clip with lane targets remapped to the tmp id, plays it once, cleans up on `clipFinished`. Adds `clipFinished` emission to `track_clips/playback.ts`. Tmp entities are in-memory only, never written to SQLite; broadcast via new `tmp_entity_added`/`tmp_entity_removed` WS messages plus the override bus. See [spawn.md](modules/spawn.md). |
| Text rendering (3D) | WIP (Phase 1) | New scene-node kinds `text_troika` (SDF via `troika-three-text`) and `text_canvas` (rasterised, optional sanitised HTML via `html2canvas` for emote support). Both honour billboarding. See [scene-graph.md](modules/scene-graph.md). |
| Opacity (compose + 3D) | WIP (Phase 1) | New `opacity` paramPath on both target kinds. Compose: merged into `LayerView` `layerStyle`. 3D: added to `components.transform` (default 1); `Viewport.tsx` walks descendant meshes once per frame to set `material.transparent`/`material.opacity` with caching. Animatable via track clips and runtime overrides. |
| New signal nodes (Phase 1) | WIP | `set_scene_node_param`, `set_compose_layer_param`, `set_text`, `start_clip` (canonical generalisation of `track_clip_trigger`; old kind retained for back-compat), `spawn_clip`, `random`. See [signal-graph.md](modules/signal-graph.md). |
| `SpawnRef` named type | WIP (Phase 1) | New primitive in `SignalTypeMap`: `{ tmpNodeId, tmpClipId, kind: 'scene_node'\|'compose_layer' }`. `spawn_clip` outputs `Event<SpawnRef>`; `set_*_param` nodes accept an optional `spawnRef` event input that overrides `targetId` for that fire. Phase 1 special case that avoids needing generic propagation. |
| ParamPath registry (shared) | WIP (Phase 1) | `packages/shared/src/paramPaths.ts` (new) — enumerates valid paramPaths per target kind with their value type (`Float \| String \| Bool`). Consumed by the track-clip evaluator, the new `set_*_param` nodes, and the runtime override bus. See [paramPaths.md](modules/paramPaths.md). |
...
- [runtime-overrides.md](modules/runtime-overrides.md) — (WIP) parallel-to-track-clip override bus for graph-driven runtime param mutation
- [spawn.md](modules/spawn.md) — (WIP) ephemeral clip-clone spawning; tmp scene-node / compose-layer instances driven by `spawn_clip`
- [paramPaths.md](modules/paramPaths.md) — (WIP) shared paramPath registry used by clips, runtime overrides, and `set_*_param` nodes
