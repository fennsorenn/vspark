...
| API controller manager | Implemented | `node_components/api_controller/` (source dir kept) — REST-driven animation queue + blendshapes; first behavior with a public REST control surface |
...
- [component-managers.md](modules/component-managers.md) — Behavior managers (VMC, breathing, lipsync, tracking, api_controller); lifecycle pattern. (Filename kept; managers live in the `node_components/` source dir, which also kept its name.)
