...
| Signal node re-architecture (class-instance / decorator model) | WIP (Phase 2, branch `feature/signal-graph-nodes-v2`) | Signal nodes move from static pure classes (`static inputPorts/outputPorts` + `static execute`) to **live class instances** whose decorated members ARE their ports: `@eventIn` method (reaction body, engine subscribes it to upstream), `@valueIn`/`@listIn` field holding a pull-thunk, `@eventOut` field holding an engine-instrumented `Emitter<T>`, `@valueOut` field holding a node-defined thunk. Base `abstract class Node` exposes DB-backed `getState`/`setState`; `reconcile()` stays rebuild-from-scratch. The engine (`signal/engine.ts`) shrinks from central dispatcher to **wiring + lifecycle** over Node instances; instrumented Emitter/thunk wrappers preserve `_edgeStates` flash/monitoring, the `enabled` check, and per-node try/catch isolation. Coupled with the type-inference change below. Repo runs TC39 Stage-3 decorators (TS 5.9, no `experimentalDecorators`). See [signal-graph.md](modules/signal-graph.md). |
| Signal graph type inference | WIP (Phase 2, branch `feature/signal-graph-nodes-v2`) | Edge-time structural type inference, with transport folded INTO the type (clean break: `PortKind`/`PortDecl.kind` DELETED). New `packages/shared/src/signal_types.ts` — `ResolvedType` AST (`primitive | record | event | list | unknown`; `unknown` is the former `'Any'`) + `isAssignable` (structural width subtyping on records, `unknown` wildcard, plus one special case: a `List<E>` target accepts source `E` or `List<E>`). New `packages/shared/src/inference.ts` — `InferGraph` (`tryAddEdge` with forward propagation + transactional rollback, `removeEdge`, `portsOf`). `SignalNodeClass` gains optional `inferPorts`; `fromDescriptor` becomes a replay over `tryAddEdge` (eliminates cycle handling). Frontend recomputes the SAME shared inference (shared `INFER_BY_KIND` table) for dynamic typed ports + drag-time validation. See [signal-graph.md](modules/signal-graph.md). |
| Phase 2 signal nodes (dynamic ports + class-instance migration) | WIP (Phase 2) | Prototype-gated, built first: `pack_event` (DYNAMIC user-named input fields via `config.fields` names-only, type inferred from connections, trailing empty slot; outputs `Event<{...}>`), `queue_events` (FIFO via setState, `popped` mirrors `enqueue` payload type, `size` value-out), `unpack_event` rewrite (DYNAMIC outputs — one typed port per record field of the resolved payload, else single `value: unknown` fallback). Then the remaining ~51 nodes migrate to the new class-instance form. **Dynamic ports** need no new decorator machinery: decorations are the static skeleton, `inferPorts` declares the actual current ports (may have no decorated member); three additive base-class accessors route by-name through a per-node dynamic port table — `this.input(name)` (pull dynamic value-in), `this.emitOn(name, v)` (push dynamic event-out), `this.output(name, fn)` (register dynamic value-out thunk). |
| Typed `component_config` | Deferred (Phase 2, out of scope) | `inferPorts`-based typing of `component_config` outputs is **deferred** — there is no config-schema registry yet, so writable graphs simply reject the node. Stays planned for a later phase. Also deferred: typed `set_*_param` value input, and incremental `reconcile`. |
