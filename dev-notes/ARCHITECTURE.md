...
| Signal node registry | Implemented | `signal/registry.ts` — 56 node kinds (mediapipe converters + IK, runtime mutation primitives `random` / `start_clip` / `spawn_clip` / `set_scene_node_param` / `set_compose_layer_param` / `set_text` / `set_data`, `log` debug, plus 13 overlive event nodes + `overlive_chat_feed`) |
...
| Video & audio assets | WIP | First-class video + audio assets: a `video` scene-node kind (3D `VideoTexture`, mirrors `billboard`), an `audio` scene-node kind (non-visual, `THREE.Audio`/`PositionalAudio`, modelled on `light`), the existing `video` compose-layer finished, a fire-and-forget **media-command bus** (graph→frontend, fourth sibling of broadcast/runtime-overrides/data-channels), a `media_control` signal node, and a track-clip **event/marker lane** for timed triggers. Scene-node kinds are schema-free at the DB layer (JSON `components` blob) so no migration for `video`/`audio`; the event lane adds a new table. See [plans/video-audio-assets.md](plans/video-audio-assets.md). Affects [scene-graph.md](modules/scene-graph.md), [compose.md](modules/compose.md), [asset-management.md](modules/asset-management.md), [track-clips.md](modules/track-clips.md), [signal-graph.md](modules/signal-graph.md). |
...
- [signal-graph.md](modules/signal-graph.md) — engine (class-instance/decorator model + edge-time type inference), all 56 node kinds, how to add a new node
...
- media (WIP) — video + audio assets, `video`/`audio` scene-node kinds, the media-command bus, and the track-clip event/marker lane. `modules/media.md` is not written yet; see [plans/video-audio-assets.md](plans/video-audio-assets.md).
