...
| Data channels + template feed layer | Implemented (Phase 3) | `data_channels/manager.ts` — generic graph→frontend publish bus, sibling of the override bus, keyed by channel name. `set_data` node publishes an arbitrary payload to a named channel; WS `data_channel_set/clear/snapshot` (snapshot on connect). Chat-specific half: a bounded per-account chat ring-buffer in `OverliveManager` + `overlive_chat_feed` node (`update` event + `messages` list). Frontend `feed` compose layer (`ComposeLayerStack.FeedLayer`) subscribes to a channel and renders the payload through a user-authored **JSX-ish (htm) template** (payload exposed as `data`; loop with `${data.map(...)}` → real React elements + keyed reconciliation), with a separate scoped `css` field and an `<Emote>` helper (DOMPurified) for raw emote HTML; templates run via `new Function` (local-use trade-off). Data-shape-independent. Shared by Editor + ViewerPage. See [data-channels.md](modules/data-channels.md). |
