...
- Re-routing the existing browser-source `obs_set_scene`/`obs_control` nodes through
  ...
  > **Reversed, and done.** `obs_set_scene` turned out to fail *silently* unless
  > the browser source's page permission is set to "Advanced" — OBS simply does
  > not expose `setCurrentScene` below that, so the call no-ops with no log and
  > no error. obs-websocket needs no such permission and returns a status. Every
  > OBS control and state node now runs over obs-websocket and the browser bridge
  > is deleted; see
