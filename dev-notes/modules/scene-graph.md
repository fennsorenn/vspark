...
| `video` | **WIP** — 3D video plane backed by `THREE.VideoTexture`, mirrors `billboard`; per-instance autoplay/loop/onEnd + muted/volume. See [plans/video-audio-assets.md](../plans/video-audio-assets.md). |
| `audio` | **WIP** — non-visual audio source (`THREE.Audio` simple / `THREE.PositionalAudio` directional), modelled on `light`; editor-muted by default. See [plans/video-audio-assets.md](../plans/video-audio-assets.md). |
