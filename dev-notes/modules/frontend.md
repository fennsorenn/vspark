...
The center view is bound strictly to the left-dock tab (`leftTab`, `Editor.tsx`): **Stage** → 3D `Viewport` (kept mounted, just hidden under other tabs, to preserve the WebGL context), **Logic** → `SignalGraphCanvas` (or a placeholder when no logic is open), **Compose** → `ComposeView`. The bottom dock shows the signal `NodePalette` on the Logic tab and the `AssetManager` otherwise. Opening any logic routes through `setActiveLogic`, which also switches `leftTab` to `'logic'` (see [project-graphs.md](project-graphs.md)).
