...
**Partial tracking**: when the avatar node's `poseSource` deviates from the legacy all-`{anim:1, track:1}` default (and blend mode is `override`), Step 2 runs a unified per-bone path that blends animation vs. live tracking independently per body section via a `BONE_TO_SECTION` map — see [animation.md](animation.md) (Partial tracking).
...
- **Expression / morph-target name lists come from persistent asset metadata.** The expression and morph-target name lists (and the VMC-receiver suggestion lists) now read from `AssetFile.metadata` keyed by `node.filePath`, falling back to the live VRM data only when the viewport hasn't loaded the avatar — via helpers `assetMetaForNode(node)` / `liveOrMetaList(...)`. This is the same **live-vs-metadata fallback pattern** the SceneGraph bone button uses (see [scene-graph.md](scene-graph.md)) and decouples these lists from the async GLTF load, so they populate for avatars in a non-active scene. **Material editing intentionally stays on the live VRM** (it mutates `THREE.Material`) and is NOT sourced from metadata.
