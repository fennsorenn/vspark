...
- **Persist-mode failure.** If `persist: true` is requested but the write-through path fails (or the hook isn't wired), the in-bus value is kept and a `console.warn` is logged. No automatic rollback — automations that need a persisted edit should treat this as best-effort.
