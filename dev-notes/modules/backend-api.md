...
| `035_tracking_grace_period_to_node.ts` | Moves the tracking-loss window from the behavior to the avatar node (run-fn, idempotent): lifts `behaviors.config.poseTimeout` (seconds) onto `scene_nodes.properties.trackingGracePeriod`, taking the largest value where several behaviors on one node disagree, and strips the dead config key. Skips nodes that already carry `trackingGracePeriod`; ignores non-numeric values. See [animation.md](animation.md) (Tracking-loss grace period). |
