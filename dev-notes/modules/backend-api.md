...
| [routes/assets.ts](../../packages/backend/src/routes/assets.ts) | Project asset upload + listing (runs `discoverAssets()` on GET). **WIP:** on VRM/GLB upload, extracts asset metadata via `vrm/metadata.ts` + records `file_mtime`, stored on the inserted `asset_files` row. |
...
| `034_asset_metadata.sql` (WIP) | Adds `metadata TEXT` (JSON `VrmAssetMetadata`) + `file_mtime TEXT` (nullable) to `asset_files`. Populated by the VRM/GLB metadata extractor at upload + on hash-drift in `discoverAssets`. Write the `.sql`; the `.ts` mirror is auto-generated by `scripts/buildMigrations.mjs`; register in `db/index.ts`. |
