...
function buildMelFilters(sampleRate: number, fftSize: number): Float32Array[] {
  ...
  for (let m = 1; m <= N_MEL_FILTERS; m++) {
    ...
    const lo = points[m - 1], ctr = points[m], hi = points[m + 1]
...
function computeMfcc(
  ...
  for (let m = 0; m < N_MEL_FILTERS; m++) {
    ...
    let sum = 0
    for (let k = 0; k < powerSpectrum.length; k++) sum += powerSpectrum[k] * f[k]
...
const DEFAULT_TEMPLATES: VowelTemplates = {
  A: [56.88148746934048, 8.695429225300636, 6.055128915365353, -7.0817173430804425, -2.4147463974564563, 18.69047922866289, -6.545224977094073, 2.184106258350576, 2.4409438615101675, 5.2020612844201, 1.2209744369177924, -2.498881573940425],
  E: [39.74647612798782, 15.735643356565447, 28.36104753282335, 9.959775394863552, -6.779232064882913, 1.4534635009273653, -8.736671840387677, 5.256605760918723, -7.1868654621972015, -5.931885159204877, 4.916308163177395, -5.3345912955109975],
  I: [33.45545895894369, 25.71246419129548, 39.46388550746588, 12.783014215069052, -9.384085984141738, 9.684172247662957, -2.8081909190334473, -1.1757180645234053, -9.031330964447545, -5.764947701383521, 3.138291744492299, -4.9273853375587855],
  O: [60.7946743103395, 32.01150223146002, 14.062001366213144, -8.337574378553644, -7.2142427111246485, 7.663210330239261, 1.4449080648910568, 0.1353482171300174, -0.3008058024626476, -5.26569019922291, -1.6022887387458853, -0.6465266349175559],
  U: [56.83788716634115, 29.782737172444662, 17.373394711812338, 0.05835628069471639, -1.2168972878654782, 3.374967514673869, -2.1147060124203563, 0.7931957999678951, 0.6637996393690463, -4.304805822372438, -0.9514333840211285, -1.0953661604908524],
}
...
export class MicCapture {
  ...
  private captureCallback: ((mfcc: Float32Array, rms: number) => void) | null = null
  ...
  async start(deviceId?: string): Promise<void> {
    ...
    this.fftBuf  = new Float32Array(this.analyser.frequencyBinCount) as Float32Array<ArrayBuffer>
    this.timeBuf = new Float32Array(this.analyser.fftSize)            as Float32Array<ArrayBuffer>
  ...
  async stop(): Promise<void> {
    ...
    this.ctx = this.analyser = this.source = this.stream = this.fftBuf = this.timeBuf = null
  ...
  private processFrame(): void {
    ...
    let sumSq = 0
    for (let i = 0; i < this.timeBuf.length; i++) sumSq += this.timeBuf[i] * this.timeBuf[i]
    ...
    const sims = VOWEL_KEYS.map(v => -distSq(liveCentred, this.prepared.centred[v]))
