...
import type { ComposeLayerOverride, NodeTransformOverride } from '../store/editorStore'
import type { TrackClipLaneRecord, TrackClipMode, TrackClipRecord } from '../api/client'
import { evaluateLane, resolveClipTime } from '../components/editor/trackClipEvaluator'
    ...
    const tick = () => {
      ...
      if (playbackEntries.length === 0) {
        if (Object.keys(s.nodeTransformOverrides).length > 0) {
          for (const id of Object.keys(s.nodeTransformOverrides)) s.setNodeTransformOverride(id, null)
        }
        if (Object.keys(s.composeLayerOverrides).length > 0) {
          for (const id of Object.keys(s.composeLayerOverrides)) s.setComposeLayerOverride(id, null)
        }
        return
      }

      const clipById = new Map<string, TrackClipRecord>(s.trackClips.map((c) => [c.id, c]))
      ...
      for (const nodeId of prevNodeIds) s.setNodeTransformOverride(nodeId, null)
      ...
      for (const layerId of prevLayerIds) s.setComposeLayerOverride(layerId, null)
...
function applyLaneResult(
  ...
  if (lane.targetKind === 'scene_node') {
    ...
    if (base == null) return
    const absolute = mode === 'relative' ? base + (rawValue - lane.defaultValue) : rawValue
    let acc = nodeAcc.get(lane.targetId)
    if (!acc) { acc = {}; nodeAcc.set(lane.targetId, acc) }
...
function readNodeParam(node: { components: Record<string, unknown> }, paramPath: string): number | null {
...
function writeNodeParam(acc: NodeAccumulator, paramPath: string, value: number): void {
  const [group, axis] = paramPath.split('.')
  if (axis !== 'x' && axis !== 'y' && axis !== 'z') return
  if (group === 'position') { acc.position ??= { x: undefined, y: undefined, z: undefined }; (acc.position as V3)[axis] = value; return }
  if (group === 'rotation') { acc.rotation ??= { x: undefined, y: undefined, z: undefined }; (acc.rotation as V3)[axis] = value; return }
  if (group === 'scale')    { acc.scale    ??= { x: undefined, y: undefined, z: undefined }; (acc.scale    as V3)[axis] = value; return }
}

function pruneAxes(v: Record<'x' | 'y' | 'z', number | undefined>): { x?: number; y?: number; z?: number } {
