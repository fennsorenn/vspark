...
function makeHTML(draw: (ctx: CanvasRenderingContext2D, size: number) => void): string {
  const size = 64
...
function makeStar(): string {
  return makeHTML((ctx, s) => {
    ...
    // Bright center
    const center = ctx.createRadialGradient(half, half, 0, half, half, half * 0.18)
...
function makeRing(): string {
  return makeHTML((ctx, s) => {
    ...
    const grd = ctx.createRadialGradient(half, half, inner * 0.8, half, half, outer)
