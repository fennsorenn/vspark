...
const S = {
  ...
  title: { flex: 1, fontWeight: 600, fontSize: 12, color: '#e0e0e0' } as React.CSSProperties,
  iconBtn: {
    background: 'none',
    border:     'none',
    color:      '#888',
  ...
  row: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 } as React.CSSProperties,
  ...
  levelBar: { flex: 1, height: 6, background: '#2a2a2a', borderRadius: 3, overflow: 'hidden', cursor: 'default' } as React.CSSProperties,
  visemeRow: { display: 'flex', gap: 4, flexWrap: 'wrap' as const, marginTop: 4 },
  ...
  canvas: { width: '100%', height: 180, display: 'block', background: '#111', borderRadius: 4 } as React.CSSProperties,
  checkRow: { display: 'flex', gap: 10, marginTop: 4, color: '#888', fontSize: 11 } as React.CSSProperties,
}
...
const VISEME_KEYS = ['jawOpen', 'Fcl_MTH_A', 'Fcl_MTH_E', 'Fcl_MTH_I', 'Fcl_MTH_O', 'Fcl_MTH_U'] as const
const VISEME_LABELS: Record<string, string> = {
  jawOpen: 'jaw', Fcl_MTH_A: 'A', Fcl_MTH_E: 'E', Fcl_MTH_I: 'I', Fcl_MTH_O: 'O', Fcl_MTH_U: 'U',
}

function VisemeDisplay({ micRef, active }: { micRef: React.RefObject<MicCapture | null>; active: boolean }) {
  const [weights, setWeights] = useState<Record<string, number>>({})
  useEffect(() => {
    if (!active) { setWeights({}); return }
    let id: number
    function poll() {
...
export function MediaInputWindow({ lipsyncComponentId, trackingComponentId, alwaysExpanded = false, ws: externalWs }: Props) {
  ...
  const resolvedLipsyncId = lipsyncComponentId
    ?? nodeComponents.find(c => c.kind === 'lipsync_processor' && c.enabled)?.id
    ?? null
  const resolvedTrackingId = trackingComponentId
  ...
  const lipsyncStatus = lipsyncActive
    ? (resolvedLipsyncId ? 'active' : 'no-component')
    : 'idle'
  ...
  return (
    <div style={S.window(pos.x, pos.y)}>
      ...
      <div style={S.titleBar} onMouseDown={onMouseDownBar}>
        ...
        {lipsyncStatus === 'active' && <span style={{ fontSize: 10, color: '#4ade80' }}>Lipsync</span>}
        {lipsyncStatus === 'no-component' && <span style={{ fontSize: 10, color: '#fbbf24' }}>No component</span>}
        {trackingActive && <span style={{ fontSize: 10, color: '#60a5fa' }}>Tracking</span>}
        {!alwaysExpanded && (
          <button style={S.iconBtn} onClick={() => setExpanded(v => !v)} title={expanded ? 'Collapse' : 'Expand'}>
            {expanded ? '−' : '+'}
          </button>
        )}
      </div>
          ...
          <div style={{ ...S.section, borderBottom: 'none' }}>
            ...
            <div style={S.row}>
              ...
              <label style={{ fontSize: 11, color: '#888', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                <input type='checkbox' checked={showPreview} onChange={e => setShowPreview(e.target.checked)} style={{ cursor: 'pointer' }} />
                Preview
              </label>
            </div>
            <div style={S.checkRow}>
              {([ ['Face', enableFace, setEnableFace], ['Pose', enablePose, setEnablePose], ['Hands', enableHands, setEnableHands] ] as const).map(([label, val, setter]) => (
                <label key={label} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                  <input type='checkbox' checked={val} disabled={trackingActive}
                    onChange={e => setter(e.target.checked)} style={{ cursor: 'pointer' }} />
                  {label}
                </label>
              ))}
            </div>
