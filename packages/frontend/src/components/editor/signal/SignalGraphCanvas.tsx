...
import {
  ...
  applyNodeChanges,
  type Node, type Edge, type NodeTypes, type EdgeTypes, type Connection, type NodeChange, type EdgeChange,
} from '@xyflow/react'
...
function SignalGraphCanvasInner({ graphId, kindMeta }: Props) {
  ...
  const setActiveGraphWritable = useEditorStore((s) => s.setActiveGraphWritable)
  ...
  useEffect(() => () => { setActiveGraphWritable(false) }, [setActiveGraphWritable])
  ...
  const mutateDescriptor = useCallback((mut: (d: GraphDescriptor) => GraphDescriptor) => {
    ...
    persistTimer.current = setTimeout(() => {
      ...
      void api.updateProjectGraph(graphId, {
        descriptor: { id: next.id, label: next.label, readonly: false, nodes: next.nodes, edges: next.edges },
      }).catch((e) => { console.error('[SignalGraphCanvas] persist failed:', e) })
    }, 400)
  }, [graphId])
  ...
  const handleNodesChange = useCallback((changes: NodeChange[]) => {
    ...
    ) as Array<NodeChange & { id: string; position: { x: number; y: number } }>
    const removals = changes.filter((c) => c.type === 'remove') as Array<NodeChange & { id: string }>
    ...
    mutateDescriptor((d) => {
      ...
      if (positionEnds.length > 0) {
        ...
        nodes = nodes.map((n) => byId.has(n.id) ? { ...n, position: byId.get(n.id)! } : n)
      }
      if (removals.length > 0) {
        const removeIds = new Set(removals.map((c) => c.id))
        nodes = nodes.filter((n) => !removeIds.has(n.id))
        edges = edges.filter((e) => !removeIds.has(e.fromNodeId) && !removeIds.has(e.toNodeId))
      }
  ...
  const handleEdgesChange = useCallback((changes: EdgeChange[]) => {
    ...
    if (!writableRef.current) return
    const removals = changes.filter((c) => c.type === 'remove') as Array<EdgeChange & { id: string }>
    if (removals.length === 0) return
    const removeIds = new Set(removals.map((c) => c.id))
    mutateDescriptor((d) => ({
      ...d,
      edges: d.edges.filter((e) => !removeIds.has(`${e.fromNodeId}:${e.fromPort}:${e.toNodeId}:${e.toPort}`)),
    }))
  ...
  const handleConnect = useCallback((conn: Connection) => {
    ...
    mutateDescriptor((d) => {
      ...
      if (d.edges.some((e) => `${e.fromNodeId}:${e.fromPort}:${e.toNodeId}:${e.toPort}` === key)) return d
      ...
      const edgeKind: GraphEdgeDescriptor['kind'] = dstPort?.portKind === 'list' ? 'list' : srcKind
      return { ...d, edges: [...d.edges, { fromNodeId: conn.source!, fromPort, toNodeId: conn.target!, toPort, kind: edgeKind }] }
    })
  ...
  useEffect(() => {
    const onLiteralChange = (e: Event) => {
      const ce = e as CustomEvent<{ graphId: string; nodeId: string; portName: string; value: unknown }>
      const d  = ce.detail
      ...
      mutateDescriptor((g) => ({
        ...
        nodes: g.nodes.map((n) =>
          n.id === d.nodeId
            ? { ...n, defaultConfig: { ...(n.defaultConfig ?? {}), [d.portName]: d.value } }
            : n,
        ),
      }))
    }
    window.addEventListener('vspark:project-graph-literal', onLiteralChange as EventListener)
    return () => window.removeEventListener('vspark:project-graph-literal', onLiteralChange as EventListener)
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    const newNode: GraphNodeDescriptor = { id, kind, position, defaultConfig: {} }
  ...
  const handleReadonlyNodesChange = useCallback((changes: NodeChange[]) => {
