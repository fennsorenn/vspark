...
function TimelineEditor({
  ...
  const selectedLane = selected
    ? (clip.lanes.find((l) => l.id === selected.laneId) ?? null)
    : null;
  const selectedKeyframe =
    selectedLane?.keyframes.find((k) => k.id === selected!.keyframeId) ?? null;
    ...
    >
      ...
      >
        ...
        <input
          ...
          onChange={(e) =>
            handlePatchClip({
              duration: Number(e.target.value) || clip.duration,
            })
          }
        ...
        {clip.lanes.map((lane) => (
          <LaneRow
            ...
            onSelectKey={(keyframeId) =>
              setSelected({ laneId: lane.id, keyframeId })
            }
            onClearSelection={() => {
      ...
      {/* Footer: add lane */}
      <div
        style={{ borderTop: '1px solid #2a2a2a', padding: 8, flexShrink: 0 }}
      >
        {adding ? (
          <AddLanePicker
            nodes={nodes}
    ...
    >
      ...
      {kf.easing === 'bezier' && (
        <span style={{ color: '#666' }}>
          in (Δt%={fmtPct(kf.inHandleTFraction)}, Δv%=
          {fmtPct(kf.inHandleVFraction)}) &nbsp;·&nbsp; out (Δt%=
          {fmtPct(kf.outHandleTFraction)}, Δv%={fmtPct(kf.outHandleVFraction)})
        </span>
      )}
      <div style={{ flex: 1 }} />
...
function AddLanePicker({
  ...
  const [targetId, setTargetId] = useState<string>(
    initialKind === 'compose_layer'
      ? (selectedComposeLayerId ?? '')
      : (selectedNodeId ?? '')
  );
  const [paramPath, setParamPath] = useState<string>(
    initialKind === 'compose_layer' ? 'x' : 'position.x'
  );
...
function ScrubRuler({
  ...
  const xToT = (clientX: number) => {
    ...
    return Math.max(
      0,
      Math.min(clip.duration, (x / rect.width) * clip.duration)
    );
  };
...
function laneValueRange(lane: TrackClipLaneRecord): {
  min: number;
  max: number;
} {
  ...
  for (const k of lane.keyframes) {
    ...
    if (outEp) {
      if (outEp.value < min) min = outEp.value;
      if (outEp.value > max) max = outEp.value;
    }
    if (inEp) {
      if (inEp.value < min) min = inEp.value;
      if (inEp.value > max) max = inEp.value;
    }
  }
  if (max === min) {
    max += 1;
...
function LaneRow({
  ...
  const handleTrackPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const next: TrackClipKeyframeRecord = {
      ...draft,
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          cursor: 'crosshair',
          background:
            'repeating-linear-gradient(to right, #161616 0 1px, transparent 1px 50px)',
        }}
      >
        ...
        {size.w > 0 && (
          ...
          >
            ...
            <line
              x1={0}
              x2={size.w}
              y1={vToY(lane.defaultValue)}
              y2={vToY(lane.defaultValue)}
              stroke="#2c2c2c"
              strokeDasharray="3 3"
            />
            {segments.map((d, i) => (
            ...
            {selected &&
              lane.keyframes.map((kf) => {
                if (kf.id !== selected.keyframeId || kf.easing !== 'bezier')
                  return null;
                const { prev, next } = siblings(lane, kf);
                const { in: inEp, out: outEp } = resolveHandleEndpoints(
                  prev,
                  kf,
                  next
                );
...
function segmentPath(
  ...
  if (
    a.easing === 'bezier' &&
    a.outHandleTFraction != null &&
    a.outHandleVFraction != null &&
    b.inHandleTFraction != null &&
    b.inHandleVFraction != null
  ) {
...
function KeyframeDot({
  ...
  const handleDotDown =
    (kind: 'kf' | 'in' | 'out') => (e: React.PointerEvent<HTMLDivElement>) => {
      e.stopPropagation();
      if (e.button === 2 && kind === 'kf') {
        onDelete();
        return;
      }
      onSelect();
  ...
  const ky = vToY(kf.value);
  const color =
    kf.easing === 'step' ? '#aaa' : kf.easing === 'bezier' ? '#8af' : '#fb7';
  ...
  const showOut =
    selected &&
    kf.easing === 'bezier' &&
    kf.outHandleTFraction != null &&
    kf.outHandleVFraction != null &&
    next != null;
  const showIn =
    selected &&
    kf.easing === 'bezier' &&
    kf.inHandleTFraction != null &&
    kf.inHandleVFraction != null &&
    prev != null;
...
function cryptoId(): string {
  const c: { randomUUID?: () => string } = (globalThis.crypto ?? {}) as {
    randomUUID?: () => string;
  };
  return c.randomUUID
    ? c.randomUUID()
