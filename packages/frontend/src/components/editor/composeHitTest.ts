...
export const composeViewportRect: { current: (() => DOMRect | null) | null } = { current: null }
...
export function layerFrame(viewport: { width: number; height: number }, layer: ComposeLayerRecord) {
...
export function pointInLayer(viewport: { width: number; height: number }, layer: ComposeLayerRecord, px: number, py: number): boolean {
...
export function layersAtClientPoint(
  ...
  if (px < 0 || py < 0 || px > viewportRect.width || py > viewportRect.height) return []
