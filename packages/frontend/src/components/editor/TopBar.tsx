...
export function TopBar() {
  ...
  return (
    ...
    <div style={barStyle}>
      {/* Left */}
      <button
        style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 13, padding: 0, display: 'flex', alignItems: 'center', gap: 4 }}
        onClick={() => navigate('/')}
      >
        ← vspark
      </button>
      <span style={{ color: '#444' }}>|</span>
      <span style={{ color: '#e0e0e0', fontWeight: 500 }}>{projectName || 'Loading…'}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button style={iconBtnStyle} onClick={handleNewScene} title="New scene">+</button>
      </div>
