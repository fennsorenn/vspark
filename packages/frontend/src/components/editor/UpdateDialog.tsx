    ...
    try {
      ...
      useEditorStore.getState().setUpdateAvailable(
        ...
        status.updateAvailable && status.latestVersion
          ? { latestVersion: status.latestVersion, releaseNotes: status.releaseNotes, channel: status.channel }
          : null
      )
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid #2a2a2a' }}>
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#888', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: '0 2px' }}>×</button>
      </div>

      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {/* Version info */}
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 12, color: '#aaa' }}>
          <span>Current: <strong style={{ color: '#e0e0e0' }}>{currentVersion}</strong></span>
          {updateAvailable && updateInfo && (
            <>
              <span style={{ color: '#555' }}>→</span>
              <span>Latest: <strong style={{ color: '#f59e0b' }}>{updateInfo.latestVersion}</strong></span>
            </>
          )}
          {!updateAvailable && <span style={{ color: '#4ade80', marginLeft: 4 }}>✓ Up to date</span>}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label style={{ fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Release channel</label>
          <select
            value={channel}
            onChange={(e) => void handleChannelChange(e.target.value as UpdateChannel)}
            style={{ background: '#222', border: '1px solid #333', borderRadius: 4, color: '#e0e0e0', padding: '4px 8px', fontSize: 13 }}
          >
            {CHANNELS.map((c) => (
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 2 }}>
          <button
            onClick={onClose}
            style={{ background: 'none', border: '1px solid #333', borderRadius: 5, color: '#888', padding: '5px 14px', cursor: 'pointer', fontSize: 13 }}
          >
            Later
          </button>
          {updateAvailable && (
