...
export function cyclePickAt(x: number, y: number): void {
  ...
  for (const id of layersAtClientPoint(rect, frontLayers, x, y)) slots.push({ kind: 'layer', id })
  slots.push({ kind: '3d' })
  for (const id of layersAtClientPoint(rect, backLayers, x, y)) slots.push({ kind: 'layer', id })
  ...
  if (debug) {
    ...
    console.log('selectedLayer', store.selectedComposeLayerId?.slice(0, 6), 'selectedNode', store.selectedNodeId?.slice(0, 6))
  }
  ...
  if (currentIdx < 0 && store.selectedNodeId) {
    ...
    if (idx3d >= 0 && composeScenePicker.current?.(x, y) === store.selectedNodeId) currentIdx = idx3d
  }
  ...
  for (let step = 1; step <= n; step++) {
    ...
    if (next.kind === 'layer') {
      if (store.selectedComposeLayerId === next.id && step < n) { if (debug) console.log('skip own layer'); continue }
    ...
    if (debug) { console.log('→ 3d', hitNodeId.slice(0, 6)); console.groupEnd() }
