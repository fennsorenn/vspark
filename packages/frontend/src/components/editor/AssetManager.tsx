...
export function AssetManager() {
  const { assets, addAsset, deleteAsset, activeSceneId, addNode, projectId, selectedNodeId, nodes, updateNode: storeUpdateNode, addNodeComponent, nodeComponents, componentKinds, cameraEffects, addCameraEffect } = useEditorStore()
  ...
  const handleAddToScene = async (asset: AssetFile) => {
    ...
    try {
      ...
      if (useEditorStore.getState().nodes.every((n) => n.id !== node.id)) addNode(node)
    } catch (e: unknown) {
  ...
  const handleAddAsBillboard = async (asset: AssetFile) => {
    if (!activeSceneId) { alert('No active scene.'); return }
    try {
      const node = await api.createNode(activeSceneId, {
        parentId: null,
        name: asset.name,
        ...
        components: {
          transform: { type: 'transform', x: 0, y: 0, z: 0, rx: 0, ry: 0, rz: 0, sx: 1, sy: 1, sz: 1 },
          billboard: { facing: 'screen', backface: 'none', width: 1, height: 1, alpha: 1, textureUrl: asset.url },
        },
      })
      if (useEditorStore.getState().nodes.every((n) => n.id !== node.id)) addNode(node)
    } catch (e: unknown) {
  ...
  const handleApplyTexture = async (asset: AssetFile) => {
    ...
    const existing = (selectedNode.components?.[key] ?? {}) as Record<string, unknown>
    const components = { ...selectedNode.components, [key]: { ...existing, textureUrl: asset.url } }
    try {
  ...
  const handleApplyCameraBg = async (asset: AssetFile) => {
    if (!selectedNode) return
    const existing = (selectedNode.components?.camera ?? {}) as Record<string, unknown>
    const components = { ...selectedNode.components, camera: { ...existing, backgroundImage: asset.url } }
    try {
      await api.updateNode(selectedNode.id, { components })
      storeUpdateNode(selectedNode.id, { components })
    } catch (e: unknown) {
  ...
  const handleAddEffect = async (kind: string) => {
    ...
    if (!ek) return
    const effect = { id: newComponentId(), nodeId: selectedNode.id, kind, enabled: true, config: { ...ek.defaultConfig } }
      ...
      <div style={{
        ...
        {tab === 'components' || tab === 'effects' ? null : tab === 'models' ? (
          <>
            ...
            <input ref={modelInputRef} type="file" accept=".vrm,.glb,.gltf" style={{ display: 'none' }}
          ...
          <>
            ...
            <input ref={animInputRef} type="file" accept=".fbx,.bvh" style={{ display: 'none' }}
              onChange={(e) => { const file = e.target.files?.[0]; if (file) handleUpload(file); e.target.value = '' }} />
          </>
        ) : tab === 'images' ? (
          <>
            <button style={uploadBtn} disabled={uploading} onClick={() => imageInputRef.current?.click()}>
              {uploading ? 'Uploading…' : 'Upload Image'}
            </button>
            <input ref={imageInputRef} type="file" accept=".jpg,.jpeg,.png,.webp,.gif,.avif" style={{ display: 'none' }}
              onChange={(e) => { const file = e.target.files?.[0]; if (file) handleUpload(file); e.target.value = '' }} />
          </>
        ) : null}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        ...
        {tab === 'components' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {selectedNode && componentKinds.map((ct) => {
        ...
        {tab === 'effects' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {selectedNode && selectedNode.kind === 'camera' && CAMERA_EFFECT_KINDS.map((ek) => {
              const alreadyAdded = cameraEffects.some((e) => e.nodeId === selectedNode.id && e.kind === ek.kind)
              return (
                <div key={ek.kind} style={{
                  background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 6,
                  padding: '10px 12px', display: 'flex', gap: 10, alignItems: 'flex-start',
                }}>
                  <span style={{ fontSize: 22, lineHeight: 1, marginTop: 2 }}>{ek.icon}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: '#e0e0e0', marginBottom: 3 }}>{ek.label}</div>
                    <div style={{ fontSize: 11, color: '#666', lineHeight: 1.4 }}>{ek.description}</div>
                  </div>
                  <button
                    style={{
                      background: alreadyAdded ? '#1a2a1a' : '#1a3a1a',
                      border: 'none', color: alreadyAdded ? '#4a7' : '#5b9',
        ...
        {(tab === 'models' || tab === 'animations' || tab === 'images') && (() => {
          const list = tab === 'models' ? models : tab === 'animations' ? animations : images
          ...
          const extBadge: React.CSSProperties = {
            display: 'inline-block', background: '#2a2a2a', borderRadius: 3,
            padding: '1px 6px', fontSize: 10, color: '#888', alignSelf: 'flex-start',
          }
          return (
                ...
                <div key={asset.id} style={cardStyle}>
                  ...
                  {asset.kind === 'image' && (
                    <img src={asset.url} alt={asset.name}
                      style={{ width: '100%', height: 80, objectFit: 'contain', borderRadius: 3, background: '#111' }} />
                  )}
                  <div style={{ fontSize: 13, color: '#e0e0e0', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {asset.name}
                  </div>
                  <div style={extBadge}>{asset.name.split('.').pop()?.toUpperCase()}</div>
                  <div style={{ display: 'flex', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
                    {asset.kind === 'model' && (
                      <button style={{ background: '#1a3a5a', border: 'none', color: '#7ab', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}
                        onClick={() => handleAddToScene(asset)}>Add to Scene</button>
                    )}
                    {asset.kind === 'animation' && canApplyAnim && (
                      <button style={{ background: '#1a3a2a', border: 'none', color: '#7c9', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}
                        title={`Apply to "${selectedNode!.name}"`}
                        onClick={() => handleApplyAnimation(asset)}>Apply to {selectedNode!.name}</button>
                    )}
                    {asset.kind === 'animation' && !canApplyAnim && (
                      <span style={{ fontSize: 10, color: '#555', alignSelf: 'center' }}>Select an avatar first</span>
                    )}
                    {asset.kind === 'image' && (
                      <button style={{ background: '#1a2a4a', border: 'none', color: '#78b', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}
                        onClick={() => handleAddAsBillboard(asset)}>Add as Billboard</button>
                    )}
                    {asset.kind === 'image' && canApplyTexture && (
                      <button style={{ background: '#2a1a3a', border: 'none', color: '#a7c', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}
                        title={`Apply texture to "${selectedNode!.name}"`}
                        onClick={() => handleApplyTexture(asset)}>Apply to {selectedNode!.name}</button>
                    )}
                    {asset.kind === 'image' && canApplyCameraBg && (
                      <button style={{ background: '#1a2a1a', border: 'none', color: '#7c9', borderRadius: 4, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}
                        title={`Set as background for "${selectedNode!.name}"`}
                        onClick={() => handleApplyCameraBg(asset)}>Set as BG</button>
                    )}
                    {asset.kind === 'image' && !canApplyTexture && !canApplyCameraBg && selectedNode && (
                      <span style={{ fontSize: 10, color: '#555', alignSelf: 'center' }}>Select billboard/particle/camera</span>
                    )}
                    <button style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', fontSize: 14, padding: '0 2px' }}
                      onClick={() => handleDelete(asset)} title="Remove">×</button>
                  </div>
                </div>
              ))}
            </div>
