...
/** World-space geometric centre (bounding-box centre) of an object's renderable
 *  meshes, falling back to its origin when it has no geometry. This is the pivot
 *  rotation spins around — it's the *centre*, not the transform origin, which can
 *  sit off to one side of the geometry. */
export function objectWorldCenter(
  obj: THREE.Object3D,
  out: THREE.Vector3
): THREE.Vector3 {
  obj.updateWorldMatrix(true, true);
  _centerBox.setFromObject(obj);
