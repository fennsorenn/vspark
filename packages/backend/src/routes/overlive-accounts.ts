...
router.post('/projects/:projectId/overlive-app-credentials', (req, res) => {
  const { label, clientId, clientSecret, redirectUri } = req.body as {
    label?: string; clientId?: string; clientSecret?: string; redirectUri?: string
  }
...
router.put('/overlive-app-credentials/:id', (req, res) => {
  ...
  db.prepare(
    `UPDATE overlive_app_credentials SET
     ...
     WHERE id = ?`,
  ).run(label ?? null, clientId ?? null, clientSecret ?? null, redirectUri ?? null, req.params.id)
  const row = getAppCredential(req.params.id)
  if (!row) return res.status(404).json({ ok: false, error: { message: 'app credential not found' } })
...
router.delete('/overlive-app-credentials/:id', (req, res) => {
  getDb().prepare('DELETE FROM overlive_app_credentials WHERE id = ?').run(req.params.id)
 ...
 */
router.post('/projects/:projectId/overlive-app-credentials/copy-from/:sourceProjectId', (req, res) => {
  const db = getDb()
  ...
  for (const src of sources) {
    ...
    stmt.run(newId, req.params.projectId, src.label, src.client_id, src.client_secret, src.redirect_uri)
...
router.post('/projects/:projectId/overlive-accounts', (req, res) => {
  const { platform, label, appCredentialId, credentials, broadcasterId, broadcasterLogin } = req.body as {
    platform?: string
    label?: string
  ...
  if (!platform || !label) {
    return res.status(400).json({ ok: false, error: { message: 'platform and label are required' } })
  }
  const id = randomUUID()
  getDb().prepare(
    ...
    id, req.params.projectId, platform, label,
...
router.put('/overlive-accounts/:id', (req, res) => {
  const { label, credentials, broadcasterId, broadcasterLogin, status, statusReason, statusMessage } = req.body as {
    label?: string
    credentials?: Record<string, unknown>
...
function getAppCredential(id: string) {
  const row = getDb().prepare('SELECT * FROM overlive_app_credentials WHERE id = ?').get(id) as unknown as AppCredentialRow | undefined
...
function getAccount(id: string) {
  const row = getDb().prepare('SELECT * FROM overlive_accounts WHERE id = ?').get(id) as unknown as AccountRow | undefined
