...
router.get('/scenes/:sceneId/compose-layers', (req, res) => {
  const rows = getDb()
    .prepare(
      'SELECT * FROM compose_layers WHERE scene_id = ? ORDER BY scene_order DESC, camera_order ASC'
    )
...
router.post('/scenes/:sceneId/compose-layers', (req, res) => {
  ...
  if (resolvedSceneOrder == null) {
    const max = db
      .prepare(
        'SELECT MAX(scene_order) AS m FROM compose_layers WHERE scene_id = ?'
      )
      .get(sceneId) as { m: number | null };
  ...
  db.prepare(
    `INSERT INTO compose_layers
       (id, scene_id, camera_node_id, parent_id, name, kind, asset_id, config,
...
router.delete('/compose-layers/:id', (req, res) => {
  ...
  if (row.camera_node_id == null) {
    const camRows = db
      ...
      .all(row.scene_id, row.scene_order) as {
      id: string;
      camera_order: number;
    }[];
    if (camRows.length > 0) {
