...
import type { GraphDescriptor, GraphStateSnapshot } from '@vspark/shared/signal'

/** Node kinds that depend on the component-context system. Reject in project graphs. */
const COMPONENT_CONTEXT_KINDS = new Set(['component_config', 'component_id', 'scene_entity'])

export interface ProjectGraphRow {
  id:          string
  project_id:  string
  name:        string
  enabled:     0 | 1
...
export class ProjectGraphManager {
  ...
  list(projectId: string): ProjectGraphRow[] {
    return getDb().prepare(
      'SELECT * FROM project_graphs WHERE project_id = ? ORDER BY created_at',
    ).all(projectId) as unknown as ProjectGraphRow[]
  }

  get(id: string): ProjectGraphRow | undefined {
    return getDb().prepare('SELECT * FROM project_graphs WHERE id = ?').get(id) as unknown as ProjectGraphRow | undefined
  }

  create(input: { id: string; projectId: string; name: string }): ProjectGraphRow {
    const db = getDb()
    db.prepare(
      'INSERT INTO project_graphs (id, project_id, name) VALUES (?, ?, ?)',
    ).run(input.id, input.projectId, input.name)
    return this.get(input.id)!
  }

  update(id: string, patch: { name?: string; enabled?: boolean; descriptor?: GraphDescriptor }): ProjectGraphRow | undefined {
    const existing = this.get(id)
    if (!existing) return undefined
    const db = getDb()
    if (patch.name !== undefined) {
      db.prepare('UPDATE project_graphs SET name = ?, updated_at = datetime(\'now\') WHERE id = ?')
        .run(patch.name, id)
    }
    if (patch.enabled !== undefined) {
      db.prepare('UPDATE project_graphs SET enabled = ?, updated_at = datetime(\'now\') WHERE id = ?')
        .run(patch.enabled ? 1 : 0, id)
    }
    if (patch.descriptor !== undefined) {
      validateDescriptor(patch.descriptor)
      db.prepare('UPDATE project_graphs SET descriptor = ?, updated_at = datetime(\'now\') WHERE id = ?')
  ...
  remove(id: string): void {
    this.stop(id)
    getDb().prepare('DELETE FROM project_graphs WHERE id = ?').run(id)
  }
  ...
  startAllEnabled(): void {
    const rows = getDb().prepare('SELECT id FROM project_graphs WHERE enabled = 1').all() as Array<{ id: string }>
  ...
  getRunningDescriptors(): Array<{ id: string; descriptor: GraphDescriptor }> {
    return Array.from(this.running.entries()).map(([id, r]) => ({ id, descriptor: r.descriptor }))
  }
   ...
   */
  *iterateNodes(): Iterable<{ graphId: string; node: GraphDescriptor['nodes'][number]; projectId: string }> {
    for (const [graphId, r] of this.running) {
      const row = this.get(graphId)
      if (!row) continue
      for (const node of r.descriptor.nodes) {
        yield { graphId, node, projectId: row.project_id }
      }
    }
  }
  ...
  private _persistNodeState(graphId: string, nodeId: string, state: unknown): void {
    try {
      const row = this.get(graphId)
      if (!row) return
      ...
      getDb().prepare('UPDATE project_graphs SET node_state = ?, updated_at = datetime(\'now\') WHERE id = ?')
