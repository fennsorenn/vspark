...
export function loadVrmSkeleton(filePath: string): VrmSkeletonData {
  ...
  if (vrm1) {
    for (const [bone, info] of Object.entries(vrm1))
      boneToNode.set(bone, info.node);
  } else if (vrm0) {
    for (const b of vrm0) boneToNode.set(b.bone, b.node);
  }
  if (boneToNode.size === 0)
    throw new Error('No VRM humanoid bones found in file');
  ...
  for (const [boneName, nodeIdx] of boneToNode) {
    ...
    skeleton[boneName] = {
      localTranslation: (node.translation ?? [0, 0, 0]) as [
        number,
        number,
        number,
      ],
      localRotation: (node.rotation ?? [0, 0, 0, 1]) as [
        number,
        number,
        number,
        number,
