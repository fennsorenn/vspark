...
import { makeVmcGraphDescriptor, HEAD_CALIB_BONES } from './vmc_graph.js';
...
interface OscMsg { address: string; args: OscArg[] }

function readOscString(buf: Buffer, off: number): [string, number] {
  let end = off;
  while (end < buf.length && buf[end] !== 0) end++;
  return [buf.toString('utf8', off, end), off + Math.ceil((end - off + 1) / 4) * 4];
}
...
function parseMsg(buf: Buffer, off: number): [OscMsg | null, number] {
  try {
    ...
    for (let i = 1; i < typeTags.length; i++) {
      switch (typeTags[i]) {
        case 'f': args.push(buf.readFloatBE(cur)); cur += 4; break;
        case 'i': args.push(buf.readInt32BE(cur)); cur += 4; break;
        case 'd': args.push(buf.readDoubleBE(cur)); cur += 8; break;
        case 'h': cur += 8; break;                              // int64 — skip, not representable
        case 'b': cur += 4 + Math.ceil(buf.readUInt32BE(cur) / 4) * 4; break;  // blob — skip
        case 's': { const [s, next] = readOscString(buf, cur); args.push(s); cur = next; break; }
        case 'T': args.push(1); break;
        case 'F': args.push(0); break;
        case 'N': break;
        default:
          // Unknown type tag — we can't know the size, so bail on this message only.
          console.warn(`[VMC] Unknown OSC type tag '${typeTags[i]}' in ${address}, skipping message`);
          return [null, off];
      }
    }
    return [{ address, args }, cur];
  } catch { return [null, off]; }
}
...
const RHYLIVE_BONES = [
  'Hips', 'LeftUpperLeg', 'RightUpperLeg', 'LeftLowerLeg', 'RightLowerLeg',  // 0-4
  'LeftFoot', 'RightFoot', 'Spine', 'Chest', 'Neck', 'Head',                  // 5-10
  'LeftShoulder', 'RightShoulder', 'LeftUpperArm', 'RightUpperArm',           // 11-14
  'LeftLowerArm', 'RightLowerArm', 'LeftHand', 'RightHand',                   // 15-18
  'LeftToes', 'RightToes', 'LeftEye', 'RightEye', 'Jaw',                      // 19-23
  'LeftThumbProximal', 'LeftThumbIntermediate', 'LeftThumbDistal',             // 24-26
  'LeftIndexProximal', 'LeftIndexIntermediate', 'LeftIndexDistal',             // 27-29
  'LeftMiddleProximal', 'LeftMiddleIntermediate', 'LeftMiddleDistal',          // 30-32
  'LeftRingProximal', 'LeftRingIntermediate', 'LeftRingDistal',                // 33-35
  'LeftLittleProximal', 'LeftLittleIntermediate', 'LeftLittleDistal',          // 36-38
  'RightThumbProximal', 'RightThumbIntermediate', 'RightThumbDistal',          // 39-41
  'RightIndexProximal', 'RightIndexIntermediate', 'RightIndexDistal',          // 42-44
  'RightMiddleProximal', 'RightMiddleIntermediate', 'RightMiddleDistal',       // 45-47
  'RightRingProximal', 'RightRingIntermediate', 'RightRingDistal',             // 48-50
  'RightLittleProximal', 'RightLittleIntermediate', 'RightLittleDistal',       // 51-53
  'UpperChest',                                                                 // 54
];
...
export class VmcManager {
  ...
  private readonly componentSkeletons = new Map<string, VrmSkeletonData | null>();
  ...
  constructor(private readonly ws: WSSync) {
    ...
    ws.onClientConnected((client) => {
      for (const [componentId, info] of this.receivers) {
        ws.sendTo(client, 'vmc_status', { componentId, connected: info.connected });
        if (info.trackingActive !== null) {
          ws.sendTo(client, 'vmc_tracking_state', { componentId, tracking: info.trackingActive });
        }
      }
    });
  }
  ...
  private persistNodeState(componentId: string, nodeId: string, state: unknown): void {
    try {
  ...
  private getNodeConfig(componentId: string, nodeId: string): unknown {
    ...
    switch (nodeId) {
      case 'comp_id':      return { componentId };
      case 'scene_entity': return { nodeId: nodeId_ };
      case 'head_calib':   return { boneFilter: HEAD_CALIB_BONES };
      case 'arm_ik_calib': return { skeleton: this.componentSkeletons.get(componentId) ?? undefined };
    }
  ...
  startReceiver(componentId: string, port: number) {
    ...
    socket.on('message', (buf, rinfo) => {
      ...
      if (!wasConnected) {
        info.connected = true;
        console.log(`[VMC] Client connected: ${rinfo.address}:${rinfo.port} → port ${port} (component ${componentId})`);
        this.ws.broadcast('vmc_status', { componentId, connected: true, remoteAddress: rinfo.address });
      }
      ...
      for (const msg of msgs) {
        if (msg.address === '/VMC/Ext/Bone/Pos' && msg.args.length >= 8) {
          ...
          rawBones[name] = [
            msg.args[4] as number, msg.args[5] as number,
            msg.args[6] as number, msg.args[7] as number,
          ];
          ...
          if (info.prevBodyArgs.length === cur.length) {
            let diff = 0;
            for (let i = 0; i < cur.length; i++) diff += Math.abs(cur[i] - info.prevBodyArgs[i]);
      ...
      if (Object.keys(rawBones).length > 0) {
        graph.fire('vmc', 'bones', mkEvent(BoneRotations.fromRecord(rawBones), ts))
      }
  ...
  syncComponents(comps: Array<{ id: string; nodeId: string; kind: string; enabled: boolean; config: Record<string, unknown> }>) {
    const active = new Set<string>();
    for (const c of comps) {
      if (c.kind !== 'vmc_receiver' || !c.enabled) continue;
      // Restore persisted node state from the config's _nodeState namespace.
      const savedStates = (c.config._nodeState ?? {}) as Record<string, unknown>;
      const stateMap = this.nodeStates.get(c.id) ?? new Map<string, unknown>();
      for (const [nid, st] of Object.entries(savedStates)) stateMap.set(nid, st);
  ...
  private _loadSkeletonForComponent(componentId: string, sceneNodeId: string): void {
    ...
    try {
      ...
      if (!filePath) { this.componentSkeletons.set(componentId, null); return; }
      ...
      console.log(`[VmcManager] Loaded VRM skeleton for component ${componentId}: ${Object.keys(skeleton).length} bones`);
    } catch (err) {
      console.warn(`[VmcManager] Could not load VRM skeleton for ${componentId}:`, (err as Error).message);
