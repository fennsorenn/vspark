import type {
  SignalNodeClass, GraphDescriptor, PortDecl,
  NodeStateSnapshot, EdgeStateSnapshot, GraphStateSnapshot, NodeExecutionContext,
} from '@vspark/shared/signal'

type ErasedNode = {
  kind:       string
  inputPorts: ReadonlyArray<Pick<PortDecl, 'name' | 'kind'>>
  execute(inputs: Record<string, unknown>, config: unknown, ctx: NodeExecutionContext): Record<string, unknown>
}
...
export class SignalGraph {
  ...
  private readonly _fwdEdges   = new Map<string, Array<{ toNodeId: string; toPort: string }>>()
  private readonly _valueEdges = new Map<string, { fromId: string; fromPort: string }>()
  private readonly _listEdges  = new Map<string, Array<{ fromId: string; fromPort: string }>>()
  ...
  addNode(id: string, kind: string): this {
    ...
    this._nodes.set(id, {
      kind, def,
      lastInputs: new Map(), lastOutputs: new Map(),
      lastExecutedAt: null,
    })
    return this
  }

  connect(fromId: string, fromPort: string, toId: string, toPort: string): this {
    const key = `${fromId}\x00${fromPort}`
    let targets = this._fwdEdges.get(key)
    if (!targets) { targets = []; this._fwdEdges.set(key, targets) }
    targets.push({ toNodeId: toId, toPort })
    return this
  }

  connectValue(fromId: string, fromPort: string, toId: string, toPort: string): this {
    this._valueEdges.set(`${toId}\x00${toPort}`, { fromId, fromPort })
    return this
  }

  connectList(fromId: string, fromPort: string, toId: string, toPort: string): this {
    const key = `${toId}\x00${toPort}`
    let sources = this._listEdges.get(key)
    if (!sources) { sources = []; this._listEdges.set(key, sources) }
  ...
  static fromDescriptor(
    ...
    for (const e of descriptor.edges) {
      if (e.kind === 'value')     graph.connectValue(e.fromNodeId, e.fromPort, e.toNodeId, e.toPort)
      else if (e.kind === 'list') graph.connectList(e.fromNodeId, e.fromPort, e.toNodeId, e.toPort)
  ...
  fire(fromId: string, fromPort: string, value: unknown): void {
    ...
    for (const { toNodeId, toPort } of this._fwdEdges.get(key) ?? []) {
      ...
      this._edgeStates.set(edgeKey, { lastFiredAt: Date.now(), lastValue: _summarise(value) })
  ...
  getStates(): GraphStateSnapshot {
    ...
    const edges: Record<string, EdgeStateSnapshot> = Object.fromEntries(this._edgeStates)
    return { nodes, edges }
  }
...
function _summarise(v: unknown): unknown {
  ...
  if (typeof v === 'object' && 'payload' in (v as object)) {
    ...
    return { _event: true, timestamp: ev.timestamp, kind: ev.payload?.constructor?.name ?? typeof ev.payload }
  }
  if (typeof v === 'object') return { _object: true, keys: Object.keys(v as object).slice(0, 8) }
  return v
}
