...
function norm(v: V3): V3 { const l = len(v); return l < 1e-9 ? [0,0,1] : [v[0]/l, v[1]/l, v[2]/l] }
...
function rotFromTo(from: V3, to: V3): Quaternion {
  ...
  return new Quaternion(ax[0]*s, ax[1]*s, ax[2]*s, Math.cos(Math.acos(d)/2))
}
...
function buildHandFrame(pts: Landmark[], side: 'left' | 'right'): {
  ...
  palmNormal = norm([palmNormal[0]-projected[0], palmNormal[1]-projected[1], palmNormal[2]-projected[2]])
  ...
  const twistAngle = Math.atan2(dot(cross(dorsalProj, nProj), fingerAxis), dot(dorsalProj, nProj))
...
function convertHand(pts: Landmark[], side: 'left' | 'right'): NormalizedPose {
  ...
  const fingers: Array<{
    ...
    mcpBone: VRMBoneName; pipBone: VRMBoneName; dipBone: VRMBoneName
  }> = [
    { mcp: H.indexMcp,  pip: H.indexPip,  dip: H.indexDip,  tip: H.indexTip,
    ...
    { mcp: H.middleMcp, pip: H.middlePip, dip: H.middleDip, tip: H.middleTip,
    ...
    { mcp: H.ringMcp,   pip: H.ringPip,   dip: H.ringDip,   tip: H.ringTip,
    ...
    { mcp: H.littleMcp, pip: H.littlePip, dip: H.littleDip, tip: H.littleTip,
  ...
  for (const f of fingers) {
    ...
    const mcpLocal = fingerSegmentLocal(pts, f.mcp, f.pip, handToWorld, restDir)
  ...
  const thumbCmcLocal = fingerSegmentLocal(pts, H.thumbCmc, H.thumbMcp, handToWorld, restDir)
  entries.push([L ? 'leftThumbMetacarpal' : 'rightThumbMetacarpal', thumbCmcLocal])
  const thumbCmcWorld = qmul(handToWorld, thumbCmcLocal)

  const thumbMcpLocal = fingerSegmentLocal(pts, H.thumbMcp, H.thumbIp, thumbCmcWorld, restDir)
  entries.push([L ? 'leftThumbProximal' : 'rightThumbProximal', thumbMcpLocal])
  const thumbMcpWorld = qmul(thumbCmcWorld, thumbMcpLocal)

  const thumbIpLocal = fingerSegmentLocal(pts, H.thumbIp, H.thumbTip, thumbMcpWorld, restDir)
...
@SignalNode({
  ...
  description: 'Converts MediaPipe 21-point image-space hand landmarks to VRM finger joint quaternions using a hand-local coordinate frame.',
