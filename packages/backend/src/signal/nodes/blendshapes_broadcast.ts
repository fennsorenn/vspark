...
export function initBlendshapesBroadcast(ws: WSSync): void { _ws = ws; broadcastBus.init(ws) }

@SignalNode({
  label:       'Blendshapes Broadcast',
  description: 'Publishes VRM expression weights to the Broadcast Bus. The bus sums all producer slots for the entity (clamped to [0,1]) and emits a merged frame on each scene tick.',
  tags:        ['output'],
  color:       '#7a3a6a',
})
