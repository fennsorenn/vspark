...
const normV  = (v: V3): V3 => { const l = lenV(v); return l < 1e-9 ? [0,0,0] : scaleV(v, 1/l) }
...
function rotByQ(q: Q4, v: V3): V3 {
  ...
  return [vx+2*(qw*cx+qy*cz-qz*cy), vy+2*(qw*cy+qz*cx-qx*cz), vz+2*(qw*cz+qx*cy-qy*cx)]
}
...
function quatFromVecs(from: V3, to: V3): Q4 {
  ...
  if (d <= -1+1e-9) {
    const ax = normV(Math.abs(from[0]) < 0.9 ? [0,-from[2],from[1]] : [-from[2],0,from[0]])
...
interface BoneWorld { pos: V3; rot: Q4 }
...
function computeFk(
  ...
  for (let pass = 0; pass < bones.length + 1 && done.size < bones.length; pass++) {
    for (const bone of bones) {
      ...
      if (parent === null) {
        ...
        result.set(bone, { pos: [0,0,0], rot: normQ(mulQ(entry.localRotation as Q4, pose)) })
        done.add(bone)
...
interface IkResult { upperRot: Q4; lowerRot: Q4 }
...
function twoBoneIk(
  ...
  const d        = lenV(armVec)
  const reach    = Math.max(Math.abs(upperLen-lowerLen)+1e-4, Math.min(upperLen+lowerLen-1e-4, d))
  ...
  const elbowDir = hLen > 1e-4
    ? normV(addV(scaleV(armDirN, Math.cos(angA)), scaleV(scaleV(hPerp, 1/hLen), Math.sin(angA))))
    : armDirN
  ...
  const upperWorldRot = normQ(mulQ(parentRot, normQ(mulQ(upperRestRot, upperRot))))
...
const ARM_BONES = {
  left:  { upper: 'leftUpperArm',  lower: 'leftLowerArm',  hand: 'leftHand',  eye: 'leftEye',  shoulder: 'leftShoulder'  },
  right: { upper: 'rightUpperArm', lower: 'rightLowerArm', hand: 'rightHand', eye: 'rightEye', shoulder: 'rightShoulder' },
} as const

function poseToRotMap(pose: NormalizedPose): Record<string, Q4> {
  const out: Record<string, Q4> = {}
  for (const [bone, q] of pose.entries()) out[bone as string] = q.toArray() as Q4
  return out
}

function fitCalib(fk: Map<string, BoneWorld>, side: 'left' | 'right'): ArmCalib | null {
  ...
  const scale    = den < 1e-9 ? 1 : Math.max(0.1, Math.min(3, dotV(wristRel, eyeRel) / den))
...
function applyArm(
  ...
  const wristTarget = addV(shoulder, addV(scaleV(wristRel, calib.scale), calib.offset))
  ...
  const { upperRot, lowerRot } = twoBoneIk(
    shoulder, wristTarget,
    upperLen, lowerLen,
    elbowHint, parentRot, upperRestRot, armAxisLocal,
  )
...
@SignalNode({
  ...
  description: 'Calibrates arm reach by touching index finger to eye corner. Uses two-bone IK to correct reach at runtime.',
  tags:        ['calibration'],
  color:       '#5a3a9f',
})
export class ArmIkCalibration {
  ...
  static execute(
    ...
    if (triggeredPort === 'capture_left' || triggeredPort === 'capture_right') {
      const side: 'left' | 'right' = triggeredPort === 'capture_left' ? 'left' : 'right'
      ...
      if (!calib) { console.warn('[ArmIkCalibration] Missing bones for capture'); return {} as OutputsOf<typeof ArmIkCalibration> }

      const prev = ctx.getState<ArmIkState>() ?? {}
      ctx.setState({ ...prev, [side]: calib })
      console.log(`[ArmIkCalibration] Captured ${side}: scale=${calib.scale.toFixed(3)} offset=[${calib.offset.map(v => v.toFixed(3)).join(', ')}]`)
      return {} as OutputsOf<typeof ArmIkCalibration>
    }

    // ── Reset ─────────────────────────────────────────────────────────────────
    if (triggeredPort === 'reset') { ctx.setState({}); return {} as OutputsOf<typeof ArmIkCalibration> }
    ...
    const corrected = pose.map((q, bone: VRMBoneName) => {
      ...
      return ov ? Quaternion.fromArray(ov as [number, number, number, number]) : q
    })
