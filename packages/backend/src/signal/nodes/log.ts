...
@SignalNode({
  label:       'Log',
  description: 'Logs the trigger event payload and the current value-input to the backend console.',
  tags:        ['utility', 'debug'],
  color:       '#3a3a3a',
})
export class LogNode {
  ...
  static execute(
    ...
    const label = (typeof (inputs as { label?: unknown }).label === 'string'
    ...
    console.log(prefix, 'event payload:', evt?.payload, '| input:', inputs.input)
