...
@SignalNode({
  ...
  description: 'Plain chat messages. Outputs both raw text and an HTML-rendered string with inline emote <img>s.',
  tags:        ['overlive', 'input'],
  color:       '#9146ff',
})
export class OverliveChatMessage {
  ...
  static execute(
    ...
    return handleOverliveEvent<ChatMessageEvent, {
      username: string; displayName: string; text: string; html: string
      color: string; isMod: boolean; isSub: boolean; isBroadcaster: boolean
      isAction: boolean; isHighlighted: boolean; cheerAmount: number
    }>(
      inputs,
      ctx,
      (e) => ({
      ...
      { username: '', displayName: '', text: '', html: '', color: '',
        isMod: false, isSub: false, isBroadcaster: false,
        isAction: false, isHighlighted: false, cheerAmount: 0 },
