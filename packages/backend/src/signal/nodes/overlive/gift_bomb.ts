...
@SignalNode({
  ...
  description: 'Fires once per bulk gift sub event (one gifter, many recipients). For individual recipients see Overlive Subscription.',
  tags:        ['overlive', 'input'],
  color:       '#9146ff',
})
export class OverliveGiftBomb {
  ...
  static execute(
    ...
    return handleOverliveEvent<GiftBombEvent, {
      gifterUsername: string; gifterDisplayName: string
      count: number; tier: string; totalGifts: number; anonymous: boolean
    }>(
      inputs,
      ctx,
      (e) => ({
      ...
      { gifterUsername: '', gifterDisplayName: '', count: 0, tier: '', totalGifts: 0, anonymous: false },
