...
export class OverliveBan {
  ...
  static execute(
    ...
    return handleOverliveEvent<BanEvent, {
      username: string; displayName: string; moderatorName: string
      reason: string; timeoutSeconds: number; isPermanent: boolean
    }>(
      inputs,
      ctx,
      (e) => ({
      ...
      { username: '', displayName: '', moderatorName: '', reason: '', timeoutSeconds: 0, isPermanent: false },
